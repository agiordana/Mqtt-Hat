...
string FileManager::readFile(string& name) {
	...
	input.open(name.c_str(),std::ios::binary);
...
bool FileManager::isVideoFile(string path) {
	if (isfile(path)) {
		...
		for(int i = 0; i < video_ext.size(); ++i) {
			hsrv::upcase(fname);
			if (matchfile(fname, video_ext.at(i))) {
				return true;
			}
		}
	}
...
bool FileManager::isImageFile(string path) {
	if (isfile(path)) {
		...
		for(int i = 0; i < image_ext.size(); ++i) {
			hsrv::upcase(fname);
			if (matchfile(fname, image_ext.at(i))) {
				return true;
			}
		}
	} 
...
bool FileManager::isfile(string& fname, string type) {
     string fline;
     ifstream in(fname.c_str());
     if(!in.is_open()) return 0;
     in>>fline;
     in.close();
     if(fline.find(type)==string::npos) return false;
     else return true;
}

bool FileManager::isfile(string& fname) {
     ifstream in(fname.c_str());
...
bool FileManager::matchfile(string fname, string ext) {
...
string FileManager::extractBin(string execpath) {
    size_t i;
    if(execpath == "") return "";
    for(i = execpath.size()-1; i > 0 && execpath[i] != '/'; i--);
    if(i > 0) return execpath.substr(0,i);
      else return "";
}
